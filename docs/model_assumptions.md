...
| Dashboard scale constants | `GTA_BASE_FLEET=3,000,000`, `DEFAULT_SAMPLED_DRIVERS=2,500`, `MAX_SAMPLED_DRIVERS=300,000`, `HIGH_SCALE_BATCH_SIZE=25,000` | `GTA_BASE_FLEET` is a dashboard scenario scale inherited from the frontend spec, not a calibrated registered-vehicle truth claim. `MAX_SAMPLED_DRIVERS` and `HIGH_SCALE_BATCH_SIZE` are backed by the committed 300k benchmark evidence: `300,000` agents in `322.798s` using 12 batches of `25,000`. `DEFAULT_SAMPLED_DRIVERS` keeps interactive demo runs fast. | Replace `GTA_BASE_FLEET` with FSA vehicle-registration/ownership weights; tune batch size per machine memory. |
...
- `PYTHONPATH=backend uv run pytest -q`: `97 passed` in `105.81s` under Python `3.13.13` after adding committed benchmark artifact coverage; warnings are PuLP deprecations from optimizer tests.
