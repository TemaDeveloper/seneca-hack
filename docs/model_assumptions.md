...
| Source/runtime toggles | `road_graph_source`, `force_osm_download`, `charger_source`, `force_charger_download` | These are reproducibility controls, not behavioral parameters. Real-grid validation uses `road_graph_source="osm"` and `charger_source="afdc"`; fallback/proxy modes exist for deterministic unit tests and offline demos. | Keep explicit so validation can require real data and tests can run offline. |
