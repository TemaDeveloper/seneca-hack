...
- `PYTHONPATH=backend uv run pytest -q`: `92 passed` in `93.81s` under Python `3.13.13` after adding high-scale conservation totals to the batched simulator, dashboard adapter, benchmark JSON, and regression tests; warnings are PuLP deprecations from optimizer tests.
