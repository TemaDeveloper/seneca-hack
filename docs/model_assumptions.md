...
| Activity POI attractions | `activity_poi_source="auto"` uses cached activity POIs only when `activity_poi_metadata.json` proves full-FSA coverage; otherwise it falls back to deterministic FSA zone-proxy attractions. Explicit `activity_poi_source="osm"` fetches OSM POIs; `force_activity_poi_download` refreshes that cache. | The activity catalog separates `work`, `school`, `retail`, `restaurant`, `bar_nightlife`, `leisure`, `errand`, `transit_hub`, and `other`. OSM tags and land-use/building polygons provide concrete-place attraction when fetched. The metadata guard prevents limited smoke fetches from being mistaken for complete GTA evidence. The fallback is not a behavioral claim; it keeps tests/offline runs deterministic while the POI acquisition path is available. | Add municipal schools, parks, employment areas, parking/access layers, and trip-purpose/time data; then document source coverage and replace proxy weights. |
...
- Intraday real-grid smoke after adding activity-location planning: `PYTHONPATH=backend .venv/bin/python data_preparation/run_model_validation.py --real-grid --itinerary-model intraday --activity-poi-source auto --observed-targets --repeat-week --num-people 300 --seeds 101 202 303 --validation-jobs 1 --out-dir backend/data/validation/intraday_real_grid_smoke_after_activity_metadata` passed with seed pass rate `100%` and broken gate count `0`. The run used real OSM roads and AFDC chargers; activity POIs fell back to zone-proxy attractions because the current OSM POI smoke cache is explicitly marked incomplete.
- Activity-POI acquisition progress: the OSM-road chunk cache is graph-fingerprint guarded and resumable by global FSA range. Current local acquisition evidence reached `14/260` compatible FSAs with `6,035` POIs and `3,068` snapped node-attraction rows; metadata remains `complete=false`, so `activity_poi_source="auto"` still correctly falls back to zone-proxy attractions until full GTA coverage is present.
