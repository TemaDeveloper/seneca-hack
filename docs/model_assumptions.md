...
Implementation:
- `data_preparation/fetch_activity_pois.py --source pbf` parses the Geofabrik Ontario OSM PBF with `osmium`, maps activity POIs and land-use/building polygons into the hackathon FSA polygons, snaps them to the active OSM road graph, and writes `activity_pois.csv`, `activity_fsa_attractions.csv`, `activity_node_attractions.csv`, and guarded metadata.
- Current complete local activity cache: 260/260 FSAs, 148,585 mapped POIs, 2,340 FSA attraction rows, 62,014 road-node attraction rows, graph fingerprint `52ba7862fd1e7732`, PBF fingerprint `5e6911a3268e6d85`, and no missing FSA ranges.
...
| Weekly schedule | `worker_weekday_work_probability=0.84`, `student_weekday_school_probability=0.86`, `weekday_nonworker_outing_probability=0.50`, `after_work_stop_probability=0.28`, `worker_weekend_work_probability=0.12`, `weekend_outing_probability=0.66`, `weekend_second_stop_probability=0.34`; work arrive-by mean `8.95h`, school `8.35h` | Calibrated against common commute structure: weekday AM work/school, weekday PM home, weekend retail/leisure, active-day sanity, and arrive-by delay gates. The schedule-only fitted values reduced validation loss while preserving charger-access assumptions. | Fit purpose/time matrices from TTS OD records. |
| Intraday route planner | `itinerary_model="template"` by default; opt-in `itinerary_model="intraday"`; `activity_day_start_hour=4.0`; `max_stops_per_activity_day=6` | The intraday planner is a semi-Markov activity-location generator. It samples dynamic stop count, activity type, destination FSA, route, dwell, and timestamps with a 04:00 activity-day home-closure rule. It is opt-in until full real-grid validation is completed because the current transition/time-window curves are assumption-backed rather than fitted from trip diaries. | Make `intraday` the default only after multi-seed validation, activity POI cache evidence, and downstream charging/grid gates pass under real OSM/AFDC data. |
| Destination choice | per-activity normalized POI attraction, purpose-zone multipliers listed below, `traffic_attraction_exponent=0.25`, distance decays `tau`: work `24 km`, school `7`, retail `10`, restaurant `9`, leisure `16`, home `20`, transit `28`, other `13`; soft caps `25-120 km`; general outing tails include transit/other | Gravity model calibrated to OSM route distribution, zone-purpose alignment, destination-share sanity, and Toronto traffic-count FSA exposure. Per-activity normalization is required with real PBF data because raw OSM category totals otherwise decide activity type; transition scores should decide purpose while POI weights decide destination. Purpose-zone multipliers counter the fact that OSM POI counts are numerically concentrated in residential FSAs. | Fit with TTS purpose/time/OD records and municipal employment/school/retail datasets. |
| Activity POI attractions | `activity_poi_source="auto"` uses cached activity POIs only when `activity_poi_metadata.json` proves full-FSA coverage; otherwise it falls back to deterministic FSA zone-proxy attractions. Explicit `activity_poi_source="pbf"` parses a local Geofabrik OSM extract; explicit `activity_poi_source="osm"` uses resumable Overpass FSA chunks. | The activity catalog separates `work`, `school`, `retail`, `restaurant`, `bar_nightlife`, `leisure`, `errand`, `transit_hub`, and `other`. Current PBF evidence maps 148,585 POIs across all 260 hackathon FSAs and 62,014 snapped road-node attraction rows on the current OSM graph. The metadata guard prevents limited smoke fetches from being mistaken for complete GTA evidence. The fallback is not a behavioral claim; it keeps tests/offline runs deterministic when real POI data is unavailable. | Add municipal schools, parks, employment areas, parking/access layers, and trip-purpose/time data; then document source coverage and replace proxy weights. |
...
| Dashboard temperature constants | `BASE_EFFICIENCY_KWH_PER_KM=0.18`, `OPTIMAL_TEMP_C=20.0`, `TEMP_EFFICIENCY_LOSS_PER_DEGREE=0.0085`, `MAX_EFFICIENCY_LOSS=0.40` | `BASE_EFFICIENCY_KWH_PER_KM` matches the simulation default and EV Database consumption reference. The temperature penalty is inherited from the legacy dashboard model and kept as a bounded scenario stressor, not a fitted weather model. | Replace with observed seasonal efficiency curves by vehicle class. |
| Dashboard scale constants | `GTA_BASE_FLEET=3,000,000`, `DEFAULT_SAMPLED_DRIVERS=2,500`, `MAX_SAMPLED_DRIVERS=300,000`, `HIGH_SCALE_BATCH_SIZE=25,000` | `GTA_BASE_FLEET` is a dashboard scenario scale inherited from the frontend spec, not a calibrated registered-vehicle truth claim. `MAX_SAMPLED_DRIVERS` and `HIGH_SCALE_BATCH_SIZE` are backed by the committed 300k benchmark evidence: `300,000` agents in `323.604s` using 12 batches of `25,000`. `DEFAULT_SAMPLED_DRIVERS` keeps interactive demo runs fast. | Replace `GTA_BASE_FLEET` with FSA vehicle-registration/ownership weights; tune batch size per machine memory. |
| Source/runtime toggles | `road_graph_source`, `force_osm_download`, `charger_source`, `force_charger_download`, `activity_poi_source`, `force_activity_poi_download`, `itinerary_model` | These are reproducibility controls, not behavioral parameters. Real-grid validation uses `road_graph_source="osm"` and `charger_source="afdc"`; fallback/proxy modes exist for deterministic unit tests and offline demos. The activity-POI fetch path is explicit because PBF parsing and Overpass/network availability should not affect normal test runs. | Keep explicit so validation can require real data and tests can run offline. |

Destination-zone multipliers used by the intraday planner:

| Activity | residential | leisure | office_park | retail_hub | transit_hub |
|---|---:|---:|---:|---:|---:|
| work | 0.55 | 0.85 | 3.20 | 2.60 | 2.00 |
| school | 1.15 | 1.20 | 1.20 | 0.75 | 0.50 |
| retail | 0.35 | 0.70 | 2.50 | 5.00 | 2.00 |
| restaurant | 0.40 | 3.00 | 2.00 | 4.50 | 2.00 |
| bar_nightlife | 0.25 | 4.00 | 0.80 | 4.00 | 2.50 |
| leisure | 1.10 | 3.00 | 0.65 | 1.80 | 1.00 |
| errand | 1.20 | 0.80 | 1.40 | 2.50 | 0.80 |
| transit_hub | 0.05 | 0.50 | 3.00 | 4.00 | 20.00 |
| other | 0.70 | 1.40 | 1.40 | 1.60 | 1.20 |

Argument: these multipliers are not destination counts. They are taxonomy-correction factors applied after real POI counts because the PBF cache places most POIs in residential FSAs by count volume. The validation target is not zero residential retail/restaurants; it is preventing residential count volume from overwhelming known retail, office, leisure, and transit centers. The accepted values passed complete-PBF real-grid validation at 500 people x seeds `101,202,303`.
...
- Intraday real-grid validation with complete activity POIs: `PYTHONPATH=backend .venv/bin/python data_preparation/run_model_validation.py --real-grid --itinerary-model intraday --activity-poi-source auto --observed-targets --repeat-week --num-people 500 --seeds 101 202 303 --validation-jobs 1 --out-dir backend/data/validation/intraday_real_grid_pbf_cache_500` passed with seed pass rate `100%` and broken gate count `0`. The run used real OSM roads, AFDC chargers, and the complete PBF activity cache. Key seed ranges: median weekly km `160.91-175.54`, route p50 `14.27-15.75 km`, route p90 `44.56-48.47 km`, transit-hub share `2.32-2.43%`, other share `0.95-1.24%`, retail zone alignment `84.99-88.70%`, restaurant alignment `86.36-90.97%`, charges `2.44-2.74/EV-week`, patches `0.12-0.15/EV-week`, and repeat-week SoC drift `-1.65` to `-0.18` percentage points.
- Activity-POI acquisition status: current local PBF cache is complete for the OSM road graph. Metadata reports `source=pbf`, `complete=true`, `260/260` FSAs, `148,585` POIs, `62,014` snapped node-attraction rows, no missing ranges, graph fingerprint `52ba7862fd1e7732`, and PBF fingerprint `5e6911a3268e6d85`.
...
- Batched conservation update: high-scale batch summaries now record charge-energy and road-flow totals (`charge_energy_kwh`, `hourly_energy_kwh`, `edge_vehicle_count`, `edge_ev_count`, `edge_route_km`). Regression tests assert these totals match the aggregated hourly and edge-flow artifacts across multiple batches. A 1,000-person benchmark smoke returned matching charge/hourly energy `10,490.989 kWh` and matching edge-route totals after three batches.
- Benchmark gate update: `data_preparation/benchmark_simulation_scale.py` now supports `--max-seconds` and `--output-json`, making the 600-second high-scale target executable and preserving machine-readable evidence. Tests cover successful JSON evidence output and threshold failure behavior.
- Assumption-ledger coverage update: regression tests now assert every `MobilityConfig` field and dashboard/runtime constant is named in this assumptions file. This prevents new behavioral or scaling constants from being added without documentation.
- Benchmark artifact coverage update: regression tests now assert `docs/benchmarks/high_scale_benchmark_300000.json` is a real-grid, 300k-person, FSA-corridor run that passed the 600-second gate and conserved charge/hourly energy.
- `PYTHONPATH=backend uv run pytest -q`: `97 passed` in `103.27s` under Python `3.13.13` after refreshing the committed benchmark artifact with command/platform/git metadata; warnings are PuLP deprecations from optimizer tests.
